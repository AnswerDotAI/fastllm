...
# %% auto #0
__all__ = ['ant_tc_types', 'norm_tool_call', 'norm_tool_calls', 'norm_usage', 'finalize_usage', 'norm_finish', 'search_hits',
...
from .types import *
from aidialog.msg_parts import (Part, PartType, mk_part, Msg, Completion, Text, Thinking, ToolUse, ToolResult, tool_text,
...
def search_hits(parts, blocks):
    "Set each server call's `text` to the `title: url` lines of its result block, when the response has one"
    calls = {p.id: p for p in parts if isinstance(p, ToolUse)}
    for b in blocks:
        if b.get('type', '').endswith('_tool_result') and (tc := calls.get(b.get('tool_use_id'))):
            tc.text = '\n'.join(f"{r.get('title', '')}: {r.get('url', '')}" for r in b.get('content') or [] if isinstance(r, dict) and r.get('url'))
    return parts

def norm_parts(resp):
    "Canonical parts of a response; a server tool's result block lives in the raw message, and as the call's `text`"
    parts = []
    for b in resp.get("content", []):
    ...
    return search_hits(parts, resp.get("content", []))
...
async def acollect_stream(resp, **kwargs):
    ...
    async for o in res:
        if isinstance(o, Completion): search_hits(o.message.content, o.message.raw['content'])
        yield o
